...
export class GetWeather extends Component {
    ...
    componentWillMount() {
        fetch(`http://api.openweathermap.org/data/2.5/weather?q=${this.state.city}&APPID=50182582175ed60ea90f269ea76e975d&units=metric`)
            .then(response => response.json())
            .then(response => 
                this.setState({degree: getDegree(response), 
                description: getDescription(response), weatherCode:
                getWeatherCode(response)}))
    }
    render() {
